...
int main(int argc, char *argv[])
{
	try
	{
		...
		manager.register_fileio(new FileIO::PosixFileFabric);
		...
		std::cout << "Test dir listing:" << std::endl;
		std::vector<std::string> dirs, files;
		manager.ListDir(".", &dirs, &files);
		std::cout<<"Found dirs: ";
		for(std::string &dir: dirs) std::cout<<dir<<" ";
		std::cout<<"\nFound files: ";
		for(std::string &file : files) std::cout<<file<<" ";
		std::cout<<std::endl;
	}
